    ...
    <n-drawer class="no-select no-drag" v-model:show="fileInfo.show" :width="350" placement="right">
      ...
      <div v-else class="pa-10">
        ...
        <div class="flex align-center pa-10">
          ...
          <div>
            {{ fileInfo.list[3] }} KB ({{ fileInfo.list[4] }} x {{ fileInfo.list[5] }})
          </div>
        </div>
        <div class="flex align-center pa-10">
...
export default {
  ...
  methods: {
    // 初始化文件管理器
    init(hostId) {
      let showHideFile = localStorage.getItem("file.show_hide");
      if (showHideFile == undefined) this.hide = true;
      else this.hide = showHideFile == "false";
    ...
    getFileList() {
      this.loading = true;
      file
        .getList(this.id, this.path, this.hide)
        .then((res) => {
          if (res.state) {
            setTimeout(() => {
              let list = [];
    ...
    getFileInfo(path) {
      ...
      file
        .getInfo(this.id, path)
        .then((res) => {
          if (res.state) {
            if (res.data[0])
              res.data[0] = res.data[0].substring(res.data[0].lastIndexOf("/") + 1);
    ...
    submitRename() {
      ...
      file
        .move(this.id, this.path + "/" + item.name, this.path + "/" + newName)
        .then((res) => {
          if (res.state) {
            ...
            window.$message.warning(
              res.message ? res.message : "重命名失败, 发生意料之外的错误"
            );
        })
    ...
    submitUpload(tips, type, path) {
      ...
      file
        .upload(this.id, type, path, this.path, name)
        .then((res) => {
          if (res.state) {
            tips.type = "success";
            tips.content = "上传成功";
    ...
    submitDirectory() {
      ...
      file
        .createDirectory(this.id, this.path + "/" + this.directory.name)
        .then((res) => {
          if (res.state) {
            ...
            window.$message.warning(
              res.message ? res.message : "目录创建失败, 发生意料之外的错误"
            );
        })
